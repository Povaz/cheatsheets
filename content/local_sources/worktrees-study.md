...
The scripts live globally under `~/.claude/scripts/spawn-agent/`, synced there by the `claude-code-dotfiles` setup flow:

```
~/.claude/scripts/spawn-agent/
  launch.sh    ← interactive shell wrapper (entry point)
  launch.py    ← worktree + branch logic
  cleanup.py   ← interactive worktree pruning
```

`launch.sh` is the entry point — it prompts for input, delegates the git setup to `launch.py`, then launches Claude with a proper interactive terminal. Nothing is committed per-project; the scripts operate on whichever repo you invoke them from (discovered via `git rev-parse --show-toplevel`).

Run the launcher from any terminal tab inside a project root:

```bash
~/.claude/scripts/spawn-agent/launch.sh
```

It prompts for three inputs:
...
Agent names can be any string matching `^[A-Za-z0-9][A-Za-z0-9._-]*$` — digits alone (`1`, `2`, `42`) are valid if you prefer the integer convention.

The script then:

1. **Checks if a worktree exists** for that agent name (e.g. `myproject-agent-3/`). If not, it creates one automatically with a placeholder branch.
2. **Checks the branch**: if it already exists and is free, switches to it. If it doesn't exist, creates it from the base branch you provided. If it's already checked out in another worktree, exits with a clear error message.
...
14:32:01  Creating new branch 'feature/task-1' from 'main'
14:32:01  Now on branch 'feature/task-1'
14:32:01  Ready: /home/you/dev/myproject-agent-3
```

Any agent name works — the script creates new worktrees on-demand, so there is no fixed limit.
...
3. Switch to **Script file** and point it to `$HOME/.claude/scripts/spawn-agent/launch.sh`
4. Set **Interpreter path** to `/bin/bash` — **not** `/bin/zsh` (PyCharm's macOS default), since `launch.sh` uses bash-only `read -p`.
5. Set **Working directory** to `$ProjectFileDir$` so the scripts discover the right repo via `git rev-parse`.
6. Check **Execute in terminal** — this opens a PyCharm terminal tab with a real TTY
7. Name it `Launch Agent` and save

Each time you trigger it, a terminal tab opens and prompts for the agent name, branch, and base branch. Because it uses `exec claude`, the terminal tab becomes the Claude session.
...
```bash
~/.claude/scripts/spawn-agent/cleanup.py
```
...
# --- CLEANUP ---
~/.claude/scripts/spawn-agent/cleanup.py   # interactive: list, select, remove worktrees
